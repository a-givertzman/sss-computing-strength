...
impl Displacement {
    ...
    pub fn new(frame_area_data: Vec<ParsedFrameData>) -> Result<Self, Error> {
        if frame_area_data.len() < 2 {
            return Err(Error::FromString(
                "Displacement new error: frame_area_data.len() < 2".to_string(),
            ));
        }
    ...
    pub fn value(&self, bound: &Bound, draft_start: f64, draft_end: f64) -> Result<f64, Error> {
        if !bound.is_value() {
            let error = Error::FromString("Displacement value error: bound is no value".to_owned());
            log::error!("{error}");
            return Err(error);
        }
    ...
    fn area(&self, pos_x: f64, draft: f64) -> Result<f64, Error> {
        let frames_first = self.frames.first().ok_or(Error::FromString(
