...
impl Windage {
    ...
    fn moment(&self) -> Result<Moment, Error> {
        Ok(self
            .area_stability
            .moment_v()?
            .scale(1. + self.icing_stab.coef_v_moment()))
    }
}
//
impl IWindage for Windage {
    ...
    fn a_v(&self) -> Result<f64, Error> {
        ...
        log::trace!(
            "\t IWindage a_v area_v:{area_v} coef:{coef} delta_area:{} result:{result} ",
            self.delta_area
        );
        Ok(result)
    }
    ...
    fn z_v(&self) -> Result<f64, Error> {
        ...
        log::trace!("\t IWindage z_v moment:{} m_vz:{m_vz} a_v:{a_v} z_v_bp:{z_v_bp} volume_shift:{} result:{result} ", self.moment()?, self.volume_shift);
        Ok(result)
    }
}
#[doc(hidden)]
...
#[allow(dead_code)]
impl FakeWindage {
    pub fn new(a_v: f64, z_v: f64) -> Self {
        Self { a_v, z_v }
    }
}
#[doc(hidden)]
