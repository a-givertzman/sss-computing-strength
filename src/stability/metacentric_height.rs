...
impl  MetacentricHeight {
    ...
    fn calculate(&self) -> Result<(), Error> {
        ...
        log::trace!("\t MetacentricHeight mass:{} shift_z:{}center_draught:{} rad_trans:{} rad_long:{} delta_m_h_ballast:{} delta_m_h_store:{} Z_m:{Z_m} H_0:{h_long_0} H:{h_long_fix} z_m:{z_m} h_0:{h_trans_0} h:{h_trans_fix} z_g_fix:{z_g_fix}", 
        ...
        *self.delta_m_h.borrow_mut() = Some(delta_m_h);
        ...
        self.parameters
            .add(ParameterID::MetacentricTransRad, self.rad_trans);
        self.parameters
            .add(ParameterID::MetacentricLongRad, self.rad_long);
        self.parameters
...
impl IMetacentricHeight for MetacentricHeight {
    ...
    fn h_long_fix(&self) -> Result<f64, Error> {
        ...
        Ok(self.h_long_fix.borrow().ok_or(Error::FromString(
            "MetacentricHeight h_long_fix error".to_owned(),
        ))?)
    }
    /// Поперечная метацентрическая высота без учета поправки   
    /// на свободные поверхности жидких грузов
    fn h_trans_0(&self) -> Result<f64, Error> {
        ...
        Ok(self.h_trans_0.borrow().ok_or(Error::FromString(
            "MetacentricHeight h_trans_0 error".to_owned(),
        ))?)
    }
    /// Поперечная исправленная метацентрическая высота
    fn h_trans_fix(&self) -> Result<f64, Error> {
        ...
        Ok(self.h_trans_fix.borrow().ok_or(Error::FromString(
            "MetacentricHeight h_trans_fix error".to_owned(),
        ))?)
    }
    /// Исправленное отстояние центра масс судна по высоте
    fn z_g_fix(&self) -> Result<f64, Error> {
        ...
        Ok(self.z_g_fix.borrow().ok_or(Error::FromString(
            "MetacentricHeight z_g_fix error".to_owned(),
        ))?)
    }
    /// Поправка к продольной метацентрической высоте на влияние
    /// свободной поверхности жидкости в цистернах балласта и запасов (2)
    fn delta_m_h(&self) -> Result<DeltaMH, Error> {
        ...
        Ok(self.delta_m_h.borrow().ok_or(Error::FromString(
            "MetacentricHeight delta_m_h error".to_owned(),
        ))?)
    }
}
///
